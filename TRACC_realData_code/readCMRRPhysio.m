...
% % %Debug for PIIN010 and PIIN006 -- this might not work -- must comment out for future
% % %use....
% % warning('Line 146 must be commented out for future use')
% % NumVolumes = 3623;
...
for curline=1:numlines
    ...
    if (strfind(line, '='))
        ...
        if (strcmp(varname, 'NumVolumes'))
            ...
            %%% added by Qiuting to fixed the NumVolumes==1 issue for
            %%% MB diffusion

%                 str = ['cat ',fn,' | tail -n 3 | head -n 1 | awk ''{print $1;}'''];
%                 [~,nvol] = unix(str);
%                 NumVolumes = str2num(nvol)+1;
